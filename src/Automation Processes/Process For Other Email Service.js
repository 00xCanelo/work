...
class OtherMailProcess {
    ...
    async execute() {
        try {
            log.info('Starting OtherMail automation process...');
            ...
            log.success('OtherMail process completed successfully!');
            return this.accountData;
            
        } catch (error) {
            ErrorHandler.handleError(error, 'automation', 'OtherMail process');
            throw error;
        }
    }
    ...
    async handleEmailVerification() {
        log.info('Handling OtherMail email verification...');
        
        try {
            await this.page.waitForSelector('[data-testid="verification-code-input"]', { timeout: 45000 });
            
            const verificationCode = await this.getOtherMailVerificationCode();
            
            if (!verificationCode) {
                throw new Error('Failed to get verification code from OtherMail');
            }
            ...
            log.success('OtherMail email verification completed');
            
        } catch (error) {
            ErrorHandler.handleError(error, 'automation', 'OtherMail email verification');
            throw error;
        }
    }

    async getOtherMailVerificationCode() {
        try {
            const code = await this.emailBot.getOtherMailVerificationCode();
            return code;
        } catch (error) {
            log.warning('OtherMail verification code retrieval failed');
            return null;
        }
    }
...
module.exports = OtherMailProcess;
